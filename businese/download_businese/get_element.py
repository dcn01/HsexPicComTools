...
class GetHsexElement(GetPage):
    ...
    def get_pic_and_title(self, url, ip_port):
        ...
        if element is not None:
            ...
            author_xpath = '//div[@id="container"]/div[@class="row body"]/div[@class!="clearfix"]/div[@class="thumbnail"]/div[last()]/p/a/text()'
            other_xpath = '//div[@id="container"]/div[@class="row body"]/div[@class!="clearfix"]/div[@class="thumbnail"]/div[last()]/p/text()'
            find_url_list = []
            find_title_list = []
            find_author_list = []
            find_other_list = []
            ...
            for n in ele_list:
                ...
                find_other_list = n.xpath(other_xpath)
                break
            ...
            for i in range(0, len(ele_list)):
                ...
                if "-" in other:
                    index_num = other.find("-")
                    other = other[index_num-4:index_num]
                else:
                    other = time.strftime("%Y", time.localtime())
                if title.strip() != '':
